...
class CommandAnalyzer(IAnalyzer.IAnalyzer):
    ...
    def neurocheck(cls, message_text, user):
        ...
        input = (
            ...
            "8, 9 - очистить текст (почисти, очисти, убери лишнее из: текст — удалить командные слова, препроцессинг)\n"
            "10 - команды нет в списке\n\n"
            "Задача: Найди команду(ы) в списке, строго учитывая контекст. "
            "Важно: «убери лишнее из: текст» — это команда 8 или 9 (очистка), не 10. Не анализируй текст на «лишние слова», это команда обработки.\n"
            ...
            "Критерии для команды 3 (нарисовать):\n"
            "- Нарисуй, нарисовать, изобрази, draw — это всегда команда 3\n"
            "- Слова после команды (нарисуй кота, нарисуй закат) — предмет рисования, не отдельная команда\n"
            "- Нарисуй ≠ напиши/создай: нарисуй = изображение = 3, напиши текст = 10\n"
            "Критерии для команды 7 (погода):\n"
            "- предскажи погоду, какая погода, скажи погоду, погода в X — это всегда команда 7\n"
            "- Любой запрос на прогноз/информацию о погоде в городе = 7\n"
            "Критерии для команды 6 (поиск в гугле/википедии):\n"
            "- Прямой запрос на поиск информации в интернете\n"
            "- Запрос должен быть направлен на получение информации из внешних источников\n"
            "НЕ считается командой 6:\n"
            "- Поиск в локальных данных (например, 'найди город в строке')\n"
            ...
            "- «убери лишнее из: X» — ВСЕГДА 8 или 9. Это команда обработки, не вопрос «есть ли лишние слова». Пример: «убери лишнее из: Мама мыла раму» → 8\n"
            "- «почисти», «очисти», «очисть», «очисти текст», «почисти текст», «удали стоп-слова» — 8 или 9\n"
            "- «очисть текст от знаков препинания и поставь слово в изначальную форму» — 8 или 9\n"
            "- Запрос на очистку/препроцессинг текста = 8 или 9. НЕ возвращай 10 для этих фраз.\n"
