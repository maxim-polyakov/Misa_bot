...
def share_chat_html(request, chat_id):
    ...
    Обычно отдаётся ботам превью (Telegram и др.) через nginx по User-Agent; люди открывают тот же URL и получают SPA.
    Прямой заход на API — ссылка в body на веб-клиент.
    """
    ctrlr = Controller.Controller()
    try:
